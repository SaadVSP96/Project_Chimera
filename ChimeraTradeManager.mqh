...
//+------------------------------------------------------------------+
//| CRITICAL: Configuration Structure (Source of Truth)              |
//+------------------------------------------------------------------+
struct ChimeraConfig {
   //--- 1. Identity & Symbols
   ulong    magic_number;           // Unique identifier for this strategy's trades
   ...
   double   balance_phase_1_limit;  // Upper limit for Foundation Phase ($1000)
   double   balance_phase_2_limit;  // Upper limit for Acceleration Phase ($5000)
   // Note: Phase 3 is anything above balance_phase_2_limit

   //--- 3. Risk Per Phase (Phased Compounding)
   double   risk_percent_p1;        // Risk for Foundation Phase (0.10 = 10%)
   double   risk_percent_p2;        // Risk for Acceleration Phase (0.15 = 15%)
   double   risk_percent_p3;        // Risk for Hyper-Growth Phase (0.20 = 20%)
   double   risk_bonus_confluence;  // Extra risk added for Score 5/5 setups (0.10)

   //--- 4. Pyramiding Limits (Max Open Positions)
   int      max_pyramids_p1;        // Max entries for Phase 1 (2)
   int      max_pyramids_p2;        // Max entries for Phase 2 (3)
   int      max_pyramids_p3;        // Max entries for Phase 3 (4)

   //--- 5. Trade Management (ATR Multipliers)
   int      atr_period;             // Period for ATR calculation (14)
   double   sl_atr_mult;            // Initial Stop Loss distance (2.0 * ATR)
   double   tp_risk_mult;           // Initial Take Profit vs Risk (1.5 R:R)
   double   trail_start_atr;        // Profit required to activate Trail (1.0 * ATR)
   ...
   int      correlation_period;     // Lookback bars for Pearson calculation (50)
   double   corr_threshold_entry;   // Minimum inverse correlation to trade (-0.6)
   double   corr_threshold_exit;    // Weak correlation level to force exit (-0.4)
   double   corr_perfect_level;     // "Perfect" level for confluence bonus (-0.8)

   //--- 7. Signal Filters & Indicators
   int      rsi_period;             // RSI Period for M5 Divergence (9)
   double   rsi_overbought;         // RSI Overbought level (60)
   double   rsi_oversold;           // RSI Oversold level (40)
   int      adx_period;             // ADX Period for H1 Trend Strength (14)
   double   adx_min_level;          // Minimum ADX value to confirm trend (25.0)
   int      ma_fast_period;         // Fast Moving Average (if used internally)
   int      ma_slow_period;         // Slow Moving Average (200 EMA)
   int      max_spread_pips;        // Maximum allowed spread (25 pips)

   //--- 8. Timeframes (Multi-Timeframe Analysis)
   ENUM_TIMEFRAMES tf_macro;        // Macro Trend (PERIOD_H4)
   ENUM_TIMEFRAMES tf_inter;        // Intermediate Trend (PERIOD_H1)
   ENUM_TIMEFRAMES tf_setup;        // Pattern Setup (PERIOD_M15)
   ENUM_TIMEFRAMES tf_exec;         // Execution Trigger (PERIOD_M5)

   //--- 9. Trading Sessions (GMT)
   int      session_london_start;   // London Open Hour (8)
   int      session_london_end;     // London Close Hour (17)
   int      session_ny_start;       // NY Open Hour (13)
   int      session_ny_end;         // NY Close Hour (22)
   
   //--- 10. Safety Constants (Previously #define)
   double   safety_equity_dd_limit;     // 20% Max Drawdown (0.20)
   double   safety_margin_level_limit;  // 300% Min Margin Level (300.0)
   double   safety_basket_dd_limit;     // 10% Max Basket Drawdown (0.10)
   
   //--- 11. Execution Settings
   int      max_deviation_points;       // Maximum price deviation in points (10)
   
   //--- 12. Advanced Pyramiding Settings
   double   pyramid_vol_mult_p2;        // Volume multiplier for pyramid 2 (0.50)
   double   pyramid_vol_mult_p3;        // Volume multiplier for pyramid 3 (0.33)
   double   pyramid_vol_mult_p4;        // Volume multiplier for pyramid 4 (0.25)
   
   double   pyramid_profit_threshold_p1; // Profit threshold for pyramid 1 in ATR (1.0)
   double   pyramid_profit_threshold_p2; // Profit threshold for pyramid 2 in ATR (2.0)
   double   pyramid_profit_threshold_p3; // Profit threshold for pyramid 3 in ATR (3.0)
   
   double   pyramid_sl_level_p1;        // SL level for pyramid 1 in ATR (0.0)
   double   pyramid_sl_level_p2;        // SL level for pyramid 2 in ATR (1.0)
   double   pyramid_sl_level_p3;        // SL level for pyramid 3 in ATR (2.0)
   
   //--- 13. Advanced Risk Management
   double   margin_requirement_multiplier; // Margin requirement multiplier (1.5)
   
   //--- 14. Take Profit Scaling
   double   tp_base_risk_mult;            // Base risk multiple for TP (1.5)
   double   tp_additional_risk_mult;     // Additional risk per trade (0.5)
};

//--- Helper Enum for Internal Logic
...
//--- Trade Journal Struct
struct CTradeJournal {
   ulong    ticket;
   datetime entry_time;
   double   entry_correlation;
   int      confluence_score;
   EnumChimeraPhase phase_at_entry;
   double   atr_at_entry;
};
