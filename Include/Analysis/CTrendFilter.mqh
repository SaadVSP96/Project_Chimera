...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CTrendFilter::CTrendFilter(const TrendFilterConfig &config) : 
    m_config(config),  // Store user configuration
...
bool CTrendFilter::IsSymbolValid() {
    if(StringCompare(m_config.symbol, "XAUUSD", false) != 0) {
        PrintFormat("CTrendFilter Error: CRITICAL - Symbol must be XAUUSD. Found: %s", m_config.symbol);
        return false;
    }
