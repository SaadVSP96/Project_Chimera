...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CChimeraTradeManager::CChimeraTradeManager(ChimeraConfig& trade_config, SFilterConfig& filter_config) {
   m_cfg = trade_config;
   m_filter_cfg = filter_config;
...
bool CChimeraTradeManager::IsSessionOpen() {
   if (!m_filter_cfg.session_filter_enabled) {
      return true;
   }
...
void CChimeraTradeManager::ExecuteEntry(ENUM_ORDER_TYPE type, double sl_price, int confluence_score, double current_correlation, datetime correlation_time) {
   ...
   switch (phase) {
      case PHASE_FOUNDATION:
         base_risk = m_cfg.risk_percent_p1;
         break;
      case PHASE_ACCELERATION:
         base_risk = m_cfg.risk_percent_p2;
         break;
      case PHASE_HYPER_GROWTH:
         base_risk = m_cfg.risk_percent_p3;
         break;
   }
   ...
   if (type == ORDER_TYPE_BUY)
      res = m_trade.Buy(lot_size, m_cfg.symbol_trade, norm_entry, norm_sl, norm_tp, "Chimera Buy");
   else
      res = m_trade.Sell(lot_size, m_cfg.symbol_trade, norm_entry, norm_sl, norm_tp, "Chimera Sell");
...
void CChimeraTradeManager::ManagePositions(double current_atr, double current_correlation, datetime correlation_time, datetime atr_time) {
   ...
   //--- Phase and Limits
   EnumChimeraPhase phase = GetCurrentPhase();
   int limit_pyramids = (phase == PHASE_FOUNDATION) ? m_cfg.max_pyramids_p1 : (phase == PHASE_ACCELERATION) ? m_cfg.max_pyramids_p2
                                                                                                            : m_cfg.max_pyramids_p3;
   ...
   if (CheckBasketDrawdown(POSITION_TYPE_BUY)) {
      ...
      if (trigger && IsSessionOpen()) {
         ...
         if (m_filter_cfg.spread_filter_enabled && current_spread > m_cfg.max_spread_pips) {
